 SELECT zip.zip_code,
   ...
   FROM zip_master zip
     LEFT JOIN ( SELECT pop_growth.geo_id,
           ...
           FROM ( WITH pop_20 AS (
                         SELECT zip_master.geo_id,
                            zip_master.total_population AS population_2020,
                            zip_master.publication_year
                           FROM zip_master
                          WHERE zip_master.publication_year = 2020::numeric
                        ), pop_10 AS (
                         SELECT zip_master.geo_id,
                            zip_master.total_population AS population_2010,
                            zip_master.publication_year
                           FROM zip_master
                          WHERE zip_master.publication_year = 2010::numeric AND zip_master.total_population > 0::numeric
                        )
                 SELECT pop_20.geo_id,
                   ...
                   FROM pop_20
                     JOIN pop_10 ON pop_20.geo_id::text = pop_10.geo_id::text) pop_growth) pg ON zip.geo_id::text = pg.geo_id::text
