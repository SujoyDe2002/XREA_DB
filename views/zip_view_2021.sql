     ...
     LEFT JOIN ( SELECT pop_growth.geo_id,
           ...
           FROM ( WITH pop_21 AS (
                         SELECT zip_master.geo_id,
                            zip_master.total_population AS population_2021,
                            zip_master.publication_year
                           FROM zip_master
                          WHERE zip_master.publication_year = 2021::numeric
                        ), pop_11 AS (
                         SELECT zip_master.geo_id,
                            zip_master.total_population AS population_2011,
                            zip_master.publication_year
                           FROM zip_master
                          WHERE zip_master.publication_year = 2011::numeric AND zip_master.total_population > 0::numeric
                        )
                 SELECT pop_21.geo_id,
                   ...
                   FROM pop_21
                     JOIN pop_11 ON pop_21.geo_id::text = pop_11.geo_id::text) pop_growth) pg ON zip.geo_id::text = pg.geo_id::text
